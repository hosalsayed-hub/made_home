    ...
    $.fn.formToWizard = function( options, cmdParam1 ) {
            ...
            $("#" + stepName + "Next").bind("click", function (e) {
                ...
                if (options.validateBeforeNext(element, $("#" + stepName)) === true) {
                    ...
                    if (stepName === "step0") {
                        ...
                        $.ajax({
                            type: "GET",
                            url: '/Site/Home/CheckPhoneNumber',
                            data: { 'PhoneNumber': $("#PhoneNumber").val(), 'CountryID': $("#CountryID").val(), 'IDNO': $("#IDNumber").val(), 'PhoneNumberStc': $("#PhoneNumberStc").val() },
                            dataType: "json",
                            contentType: "application/x-www-form-urlencoded",
                            success: function (result) {
                                debugger;
                                if (!result.status) {
                                    $("#PhoneNumberError").html(result.message);
                                    $("#PhoneNumberError").show();
                                    return false;

                                } else if (result.idnoStatus.value != true) {

                                    $("#IDNumberError").html(result.idnoStatus.value);
                                    $("#IDNumberError").show();

                                    return false;
                                }
                                else if (result.phonenumberstcStatus.value != true) {

                                    $("#PhoneNumberStcError").html(result.phonenumberstcStatus.value);
                                    $("#PhoneNumberStcError").show();

                                    return false;
                                }                                else {
                                    //$("#PhoneNumberError").html(result);
                                    //$("#PhoneNumberError").show();
                                    $("#" + stepName).hide();
                                    $("#step" + (i + 1)).show();
                                    //if (i + 2 == count)
        ...
        function selectStep(i) {
            if ( typeof(options.progress) === "function" ) {
                ...
                $("#stepDesc" + i).prevAll().addClass("done");
                $("#stepDesc" + i).next().removeClass("done");
            }
