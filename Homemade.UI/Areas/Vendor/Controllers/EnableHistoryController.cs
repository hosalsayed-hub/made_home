...
{
    public class EnableHistoryController : Controller
    {
        #region Declarations
        private readonly BlEnableHistory blEnableHistory;
        private readonly BLUser bLUser;
        private readonly BlVendor _blVendor;
        public EnableHistoryController(BlEnableHistory blEnableHistory, BLUser bLUser, BlVendor blVendor)
        {
            this.blEnableHistory = blEnableHistory;
            this.bLUser = bLUser;
            _blVendor = blVendor;
        }
        #endregion
        ...
        [CustomeAuthoriz((int)ControllerEnum.VendorEnableHistory, (int)ActionEnum.View)]
        ...
        public JsonResult LoadTable()
        {
            ...
            var data = blEnableHistory.GetAllEnableHistory(InfraStructure.Utility.CurrentLanguageCode);
        ...
        public JsonResult LoadTableVendor()
        {
            ...
            var data = blEnableHistory.GetAllEnableHistoryByVendors(VendorsID, InfraStructure.Utility.CurrentLanguageCode);
            if (Request.Form.ContainsKey("sSearch"))
            {
                string search = Request.Form["sSearch"];
                if (!string.IsNullOrEmpty(search))
